...
| `MATCHER.md` | the matcher file format, written so a person or an AI can author one without seeing the app |
| `xlsx.js` | the workbook writer behind *Export to Excel* — built on `zip.js`, no dependencies |
...
`node tests/xlsx.mjs` runs a 30-check audit of `xlsx.js`, every workbook opened by openpyxl — an independent reader — to check sheet names, cell values and types, the percent format, the bold frozen header and the font.

`node tests/matcher.mjs` runs a 73-check audit of `matcher.js`: the comparison catalogue, Soundex against its published examples, rule semantics, blank handling, the blocking a rule derives from itself, and a full run against generated data that is then scored.
...
1. Create a repo and push these files (`index.html`, `engine.js`, `scoring.js`, `matcher.js`, `zip.js`, `xlsx.js`, `docs.html`, `faker.iife.js`, `faker-ext.js`, `README.md`).
...
Every field card also gets a **keep in dups** switch whenever an entity makes duplicates: a kept field is copied unchanged into every variant whatever the level, so you decide which fields the duplicates agree on and which can vary.
...
You do not need a matching engine to use any of that. **Simulate a matcher** inside the same dialog builds one from rules: a rule is a set of field comparisons that must all pass, a pair is linked if any rule passes, and each rule carries a confidence that becomes the pair's score. Comparisons cover exact, case- and punctuation-insensitive, first-N-characters, Soundex, same-words-any-order, Jaro-Winkler, Levenshtein, Jaro-Winkler on sorted words, numeric tolerance and date tolerance. Press *Suggest rules* and the field names propose a starting set. Blocking is derived rather than configured: any equality comparison in a rule becomes its key, so a rule with one costs a fraction of the pair space and a rule without one says so before you run it. The report then adds the diagnostics only a matcher run on the page can give: a per-rule breakdown with bars (compared, linked, correct, over-matches), a per-comparison tally of what rejected each missed pair so you know which threshold to move, and a pair inspector that lays out every rule and comparison with the values measured and a pass or fail mark on each. **Variants by rule** goes further, because the generator keeps every variant's original: a table of rules against kinds of change (case, spacing, typo, digits, date shift, reformat, blank) showing how many variants of each kind every rule still caught, and a filtered list of the variants with each change spelled out and a caught-or-lost chip per rule; a *what if* panel applies the entity's own dup settings to one field of one record and shows every value it could become with a pass or fail per rule. A *link policy* decides what happens when several candidates pass for one record: link them all, or **match each record to its single best candidate** (highest score, then most rules agreed, then closest values) the way a hub matches an incoming record to one master; a *Match targets* lookup lists any record's candidates best first with its pick marked. **Export scorer** and **Import scorer** save and restore the whole configuration as a JSON file, rules and bands and fields together, so a scorer travels beside its schema file; **Save matcher** and **Import matcher**, in the rule builder, do the same for the rules alone, so the matching logic can be iterated on and shared without the rest of the dialog. The matcher file format is specified in [MATCHER.md](./MATCHER.md), written so that a person or an AI can author one without seeing the app; **Copy prompt for an AI** in the rule builder puts that specification, together with the current entity's fields, on the clipboard. **Export to Excel** writes the whole report as a workbook, one sheet per table (summary, sweep, by rule, misses by comparison, false merges, missed matches, lost to blocking, the rule configuration), with each mistaken pair annotated by the rules that matched it and the comparison that failed.
